...
CUSTOM_AUTH sign-in (OTP-T-12, `docs/specs/changes/cognito-email-otp-login`: `OTP-R-7` modified, design.md §18.1 steps 2, 6, 9). Same CLARISA `auth` header, Cognito app client and secret hash as `login/custom`. `InitiateAuth` uses `AuthFlow: CUSTOM_AUTH` (no `PREFERRED_CHALLENGE`, no `SELECT_CHALLENGE` branch — the pool's `DefineAuthChallenge`/`CreateAuthChallenge` triggers, `cognito-triggers`, always answer with a single `CUSTOM_CHALLENGE`); `RespondToAuthChallenge` uses `ChallengeName: CUSTOM_CHALLENGE` with `ChallengeResponses.ANSWER` carrying the code.

`codeDeliveryDestination` in the `login/otp/start` response is **derived locally from the request `username`** (`CognitoService.maskEmail`, design.md §13 (m)), never read from Cognito's `ChallengeParameters.destination`/`CODE_DELIVERY_DESTINATION`: `PreventUserExistenceErrors` makes an unknown user's Cognito reply come back `****@****` while a known user's is the real masked address — an enumeration tell this fixes by computing the same mask locally for every user.

| Route                         | Request                       | Success (201)                                                                                                  | Errors                                                                                                                                                                                                                                                                                                                                                                                             |
| ----------------------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /auth/login/otp/start`  | `{ username }`                | `{ challengeName: 'CUSTOM_CHALLENGE', session, codeDeliveryDestination? }`                                     | `401 { statusCode: 401, code: ATTEMPTS_EXCEEDED \| NOT_AUTHORIZED \| CHALLENGE_NOT_SUPPORTED, message, path, timestamp }` · `502 { statusCode: 502, code: UPSTREAM_ERROR, message, path, timestamp }`                                                                                                                                                                                              |
| `POST /auth/login/otp/verify` | `{ username, code, session }` | `{ tokens: { accessToken, idToken, refreshToken, expiresIn, tokenType } }` — identical shape to `login/custom` | `401 { statusCode: 401, code: CODE_MISMATCH, message, session, path, timestamp }` (wrong code — **carries the rotated Cognito `session`** for the client's retry, OTP-R-4) · `401 { statusCode: 401, code: CODE_EXPIRED \| ATTEMPTS_EXCEEDED \| NOT_AUTHORIZED \| CHALLENGE_NOT_SUPPORTED, message, path, timestamp }` · `502 { statusCode: 502, code: UPSTREAM_ERROR, message, path, timestamp }` |
